...
void compare_plot(TFile *file0, TFile *file1, const char* path, string quantity)
{
    ...
    if (quantity == "pt")
    {
        pEff0->SetTitle("Efficiency of StandAlone Probe Muon;_{p}T (GeV/c);Efficiency");
    }
    if (quantity == "eta")
    {
        pEff0->SetTitle("Efficiency of StandAlone Probe Muon;#eta;Efficiency");
    }
    if (quantity == "phi")
    {
        pEff0->SetTitle("Efficiency of StandAlone Probe Muon;#phi;Efficiency");
    }
    ...
    txCOD->SetNDC(kTRUE);
    txCOD->DrawLatex(0.14,0.85,Form("#bf{CMS DAS School 2018 Data}"));

    //Results stored in...
    const char* directoryToSave = "Comparison_Run2018_vs_MC/";
...
//Compare efficiency
void compare_efficiency(string quantity, string run2018, string MC)
{
    TFile *file1 = TFile::Open(run2018.c_str());
